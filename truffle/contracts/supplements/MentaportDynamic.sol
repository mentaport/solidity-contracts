...
contract MentaportDynamic is MentaportMintStatic {
    ...
    function tokenURI(
        ...
        require(
            _exists(tokenId),
            "MentaportDynamic: URI query for nonexistent token"
        );
        ...
        return
            bytes(currentStateURI).length > 0
                ? string(
                    abi.encodePacked(
                        currentStateURI,
                        tokenId.toString(),
     ...
     */
    function reveal() external override whenNotPaused onlyContractAdmin {
        require(
            !useDynamicRules,
    ...
    ) external whenNotPaused onlyContractAdmin {
        _recentStateSet = 0;
    ...
    /**
     * @dev Set use of dynamic rules in contract
    ...
    function useUseDynamicRules(bool _state) external onlyOwner {
        ...
        emit RuleUpdate(
            msg.sender,
            string.concat("Setting dynamic rules: ", state.toString())
        );
    }
     ...
     */
    function updateAllDynamicStates(
        uint256 _nstate,
        string memory _newBaseURI
    ) external whenNotPaused onlyContractAdmin {
        require(revealed, "Cant update,contract is not revealed");
        require(
            _recentStateSet == _nstate - 1,
        ...
        emit StateUpdate(
            ...
            _nstate,
            string.concat(
                "Updating dynamic state for all tokens: ",
     ...
     *  - Emits a {StateUpdate} event.
     */
    function updateDynamicStateURI(
        string memory _newBaseURI,
     ...
     *  - Emits a {StateUpdate} event.
     */
    function updateTokenDynamicState(
        uint256 _tokenId,
        uint256 _updateState
    ) external whenNotPaused onlyContractAdmin {
        require(revealed, "Cant update - contract is not revealed");
        ...
        require(
            _exists(_tokenId),
            "Failed, updating state for nonexistent token"
        );
        ...
        emit StateUpdate(
            ...
            _updateState,
            string.concat(
                "Updating dynamic state for token: ",
                _tokenId.toString()
            )
        );
    }
