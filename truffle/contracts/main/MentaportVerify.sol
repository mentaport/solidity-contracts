...
contract MentaportVerify is AccessControl {
    ...
    modifier onlyValidMessage(
        uint _timestamp,
        uint256 _rule,
        bytes memory _signature
    ) {
        require(
            isValidSigner(msg.sender, _timestamp, _rule, _signature),
            "Wrong signature"
        );
        _;
    }
    ...
    modifier onlyValidSigner(
        ...
        require(
            isValidSigner(signer, _timestamp, _rule, _signature),
            "Invalid signer"
        );
        _;
    }
    ...
    function isValidSigner(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(address(this), _account, _timestamp, _rule)
        );
